...
if [ -z "$UDC" ]; then
  ...
  # ── STEP 8: NCM gadget setup ────────────────────────────────────────────────
  log "STEP8: setting up NCM gadget on $UDC..."
  ...
  echo "$UDC"              > "$GADGET/UDC"
  log "STEP8: NCM gadget enabled on $UDC"
...
log "=== RECOVERY READY — connect at 10.15.19.82:23 (interactive) or :9999 (scripted) ==="
log "=== set host IP: sudo ifconfig en0 10.15.19.1 netmask 255.255.255.0 ==="
...
while [ $N -lt 60 ]; do
  sleep 30
  N=$((N+1))
  log "alive tick $N ($((N*30))s) — telnetd=$(ls /proc/$TELNET_PID 2>/dev/null && echo running || echo gone)"
